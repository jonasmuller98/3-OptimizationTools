...
dominio = [0.01 3.4];%.*ones(nvar,1); % Domínio das nvar variaveis
...
xlbest = dominio(1,2).*ones(npart,nvar); % Inicia Vetor de melhor local
...
disp('Área: 6.4516e-6 < A < 0.002193544');
disp(['Melhor Área Obtida: [',num2str(melhor_area),' m^2]']);
disp(' ');

disp('Desloc Nodal: -0.00889 < u < 0.00889');
disp(['Max Obtido: [',num2str(max(melhor_disp(:,1))),' m]; Min Obtido: [',num2str(min(melhor_disp(:,1))),' m]']);
disp(' ');

disp('Tensão: -275790291.7271 < sigma < 275790291.7271');
