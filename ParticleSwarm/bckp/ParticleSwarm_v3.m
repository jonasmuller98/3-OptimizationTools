...
dominiox=[-4.5 4.5];% Domínio na variável 1
dominioy=[-4.5 4.5];% Domínio na variável 2...
niter = 2000;    % Número de Iterações

%% Inicialização dos Vetores de Projeto
% Distribuindo as Partículas
for ivar = 1:npart             
    position(ivar,1) = dominiox(1,1)+rand*(dominiox(1,1)-dominiox(1,2));
    position(ivar,2) = dominioy(2,1)+rand*(dominioy(2,1)-dominioy(2,2));
end
