...
population = 100; npart=population;   % População total de Partículas
variables = 2; nvar=variables;     % Variáveis de Projeto
lambda1 = 2;    % Lambda 1
lambda2 = 2;    % Lambda 2
omega = 0.5;    % Propriedade de Inércia
dominio=[3 4.5;3 5.5]; % Domínio das nvar variaveis
...
nerros = 500;   % Número máximo de iterações com o erro ativado
rng(5);

for p1=0:0.1:1 
f1 = @(x) 30.*(8-x(1)) + 45.*((18-x(1)).*(8-x(2))) + 60.*(x(2).*(8-x(1)));
f2 = @(x) -x(2).*(8-x(1));
f = @(x) p1.*(30*(8-x(1)) + 45*((18-x(1))*(8-x(2))) + 60*(x(2)*(8-x(1)))) + (1-p1)*(x(2).*(8-x(1))).^2;
...
for i=1:size(store,1)
    fcalcx(i,1) = f1(store(i,:));
    fcalcy(i,1) = -f2(store(i,:));
end

scatter(fcalcx,fcalcy);
