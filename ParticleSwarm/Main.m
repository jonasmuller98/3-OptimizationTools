...
for ivar = 1:variaveis
    position(:,1) = dominiox(1)+rand*(dominiox(1)-dominiox(2));
    position(:,2) = dominioy(1)+rand*(dominioy(1)-dominioy(2));
...
while parada == 0
    ...
    for ia=1:npart
        ...
        if pos(ia,1) > dominio(2) || pos(ia,1) >  dominio(2)
            passoux(cont1,1) = pos(ia,1);
            cont1=cont1+1;
            pos(ia,1) = pos(ia,1) - (pos(ia,1)-1);
            
        elseif pos(ia,2) <  dominio(1) || pos(ia,2) <  dominio(1)
        ...
        scatter3(xgbest(1,1),xgbest(1,2),f(xgbest(1,1),xgbest(1,2)),'linewidth',1.6);hold on;
